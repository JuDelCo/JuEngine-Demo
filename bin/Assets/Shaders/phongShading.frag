...
void main()
{
	vec3 viewPos = vec3(0.0); // All operations are in camera space
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
	...
	// Colors
	vec3 diffuse = light.color * diff * (v_Color + vec3(texture(material.diffuse0, v_TexCoords)));
